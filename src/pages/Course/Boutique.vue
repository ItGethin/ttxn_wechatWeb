			...
			<li v-for="(item,index) in listData" :key=index  @click="enter(item,index)">
				...
				<div class="btqbtn" >
					...
					<div class="btqsecond" v-if="item.is_end<=0"><span class="update" v-if="item.is_end<=0">更新至第{{item.subjects}}节 </span>
						...
						<span class="nutrition" v-if="item.is_end<=0"><em class="shu"></em><strong class="troduce">{{item.audioTitle}}</strong></span>
...
export default {
  ...
  methods: {
    ...
    requireData(){
      var that = this;
      var url =
        "subject/audioSubjectList?state=1&pageSize=8&pageNum=" + this.num;
      this.$get(url, {
...
#boutique ul li .btqbtn .btqsecond .time {
  padding: .08rem .12rem .08rem .133333rem;
